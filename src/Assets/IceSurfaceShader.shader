Shader "Custom/IceSurfaceShader"
{
    Properties
    {
		...
		[PowerSlider(5.)]_TessFactor("Tess Factor",Range(1.0, 100.0)) = 30
	}
    SubShader
    {
        ...
        Pass
        {
			...
			d2f dom(h2d_const hs_const_data, const OutputPatch<h2d_main, 3> i, float3 bary:SV_DomainLocation) {
				...
				float3 pos = pos0 - _SphericScale * (2.0 * length(pos0) - 1.0) * pos0;// 少し丸くする
				...
				du = du - _SphericScale * (2.0 * length(du) - 1.0) * du;// 少し丸くする
				dv = dv - _SphericScale * (2.0 * length(dv) - 1.0) * dv;// 少し丸くする
				du = (du - pos) + nrm * Fbm(du) * _HeightScale;// 凹凸をつける 
				dv = (dv - pos) + nrm * Fbm(dv) * _HeightScale;// 凹凸をつける
				nrm = normalize(cross(du, dv));// 接ベクトルの外積から法線を求める
